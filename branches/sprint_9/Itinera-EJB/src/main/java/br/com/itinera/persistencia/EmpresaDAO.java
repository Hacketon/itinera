...
public class EmpresaDAO {
    ...
    public List<Empresa> buscarFornecedores(String nome) {
        if (nome == null) {
            return null;
        } else {
            List<Empresa> fornecedores = em.createQuery("SELECT e FROM Empresa e WHERE e.tipo = 'F' AND lower(e.nomeFantasia) like :nomeFantasia")
                    .setParameter("nomeFantasia", "%"+nome.toLowerCase()+"%")
                    .getResultList();
            return fornecedores;
        }
    }
